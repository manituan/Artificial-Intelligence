...
AIElite::AIElite(bool bIsAttacking, int type, int race, Ogre::Vector2 position, Ogre::Vector2 orientation)
{
    ...
    m_pStateMachine = new AIStateSystem<AIElite>(this);
    m_pStateMachine->SetCurrentState(SeekToFlagElite::Instance());
}

void AIElite::Update(double timeElapsed)
{
...
bool AIElite::HandleMessage(const Telegram& msg)
{
  return m_pStateMachine->HandleMessage(msg);
}

Ogre::Vector2 AIElite::seekFlag()
{
    m_iAnimation = WALK;
    if(this->IsAttacking())
    {
