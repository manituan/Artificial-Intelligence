...
AIRider::AIRider(bool bIsAttacking, int type, int race, Ogre::Vector2 position, Ogre::Vector2 orientation)
{
    ...
    m_pStateMachine = new AIStateSystem<AIRider>(this);
    m_pStateMachine->SetCurrentState(SeekToFlagRider::Instance());
}

void AIRider::Update(double timeElapsed)
{
...
bool AIRider::HandleMessage(const Telegram& msg)
{
  return m_pStateMachine->HandleMessage(msg);
}

Ogre::Vector2 AIRider::seekFlag()
{
    m_iAnimation = WALK;
    if(this->IsAttacking())
    {
